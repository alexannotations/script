...
instalar () {
    ...
    verifyInstall=$(which psql) # se almacena la verificacion de instalacion, 
    # llamando a un comando dentro de un script
    # si es igual a cero si se encontro la instruccion, si es otro valor falla
    # $?, el código de salida del pipe más reciente 
    # encadenados varios comandos mediante el carácter pipe que se escribe como |
    if [ $? -eq 0 ]; then
